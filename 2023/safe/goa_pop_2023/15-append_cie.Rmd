...
We profiled over values of $M$ from 0.01 to 0.30 in increments of 0.02 using the base model.  (Profiles on $M$ have been presented and discussed during the CIE and subsequent reviews of this model). This method involves fixing $M$ and removing the prior on $M$ from the total NLL calculation.  
...
We investigated the estimate of $M$ in the SS model, using either the original, narrow $M$ prior or the broader POP-specific FishLife prior described above (Figure 10.\@ref(fig:mposterior)). 
Key differences between the 2021 base and SS model include:
  ...
  + *Data Weighting*: we explored a version of the SS model where there are no data weights applied (all data sources' contributions to the overall objective function are equally weighted, whereas the base model weights all data sources to "1" and the catch data to "50"), as well as a version with three iterations of Francis compositional weights applied. These down-weighted the composition data as follows: fishery lengths to 0.16, fishery ages to 0.06, and  survey ages to 0.05.
 
  + *Fishery Selectivity* is specified differently in the SS model. The functional form is double normal, so that fishery selectivity can be dome shaped (as it is in the base model), logistic, or somewhere in between, given the same four time-blocks as in the base model. We explored either forcing fishery selectivity to be logistic, or allowing the descending limb to form a dome. Dome shaped selectivity resulted in a large increase in model scale (Figure 10.\@ref(fig:ssmcompare)).

Several useful findings emerged from this effort. How the $M$ prior affects model dynamics appears sensitive to fishery selectivity and whether or not data weights are included. In a model using the FishLife prior, logistic fishery selectivity and no data weighting, the SS model estimated $M$ to be `0.032` (Figure 10.\@ref(fig:mestimatess1); when data weighting was enabled for this same model, the $M$ estimate agreed more with the prior with an MLE of `0.091` (Figure 10.\@ref(fig:mestimatess2). Using the original, restrictive Hamel (2015) prior in a model with logistic fishery selectivity and no data weights resulted in an $M$ estimate nearly equivalent to the prior mean (`0.061`, Figure 10.\@ref(fig:mestimatess3)).

Recall that using the FishLife prior in the base model resulted in an estimate of $M$ to be high at `0.11` (Figure 10.\@ref(fig:mposterior)), which was illustrated further using likelihood profiles on the base model (Figure 10.\@ref(fig:mprofilebase)). Additionally, 


We suspect this difference is due to how selectivity is modeled. In an SS model where selectivity was allowed to be dome-shaped, $M$ was closer to the 2021 base value at `0.086` (Figure 10.\@ref(fig:mestimatess4) and the model scale is greatly increased (Figure 10.\@ref(fig:ssmcompare)); the base model is controlled by the gamma function to be traditionally dome-shaped. This would explain the need to constrain $M$ in the base model when the transition to gamma-shaped selectivity was made; the SS model assumes that older fish are indeed selected by the fishery, and there is information in the fishery and survey ages to suggest that $M$ is low. 

We ran likelihood profiles on $M$ using the SS model to investigate whether similar patterns in data conflict persist across the two models (Figure 10.\@ref(fig:mprofiless)). The broader, higher FishLife prior does not exert an undue influence on M, regardless of whether data weights are applied. The conflict between age and length (fishery) data persists; the recruitment trend would suggest a higher value of $M$. Inspecting the age-composition profiles by fleet reveals that most of the information is coming from the survey data (Figure 10.\@ref(fig:pinerplotmss)). Another important observation is that while the length data suggests a slightly higher value for $M$ than the SS MLE, it is still within reasonable bounds (~0.06) whereas the fishery length data in the base model suggest an MLE for $M$ outside the tested bounds (Figure 10.\@ref(fig:mprofiless)). 

### Conclusions regarding M

Overall, given the framework of the base model, it is apparent that the current prior on $M$ is reinforcing the perception given by most of the data that $M$ is between 0.04 and 0.07, and that in the absence of this prior (or in the presence of the broader FishLife prior) the base model's estimate for $M$ would be even higher. Investigations using an alternative modeling framework revealed that allowing fishery selectivity to be asymptotic results in $M$ values similar to the base case MLE, regardless of the prior used. The previous assessment author indicates that the strict prior on $M$ in the base model was a necessary compromise when the fishery selectivity was transitioned to the current gamma (dome-shaped) distribution, so it is likely that both selectivity specification and differences in how population dynamics are represented (e.g., recruitment) between the current and SS models are facilitating estimation of $M$. We conclude that it is worthwhile to continue developing the SS model with particular attention on these processes. The use of the FishLife prior (or a hybrid of the Hamel and FishLife approaches) is likely appropriate, and not unduly influential, *within the SS framework*.

We do *not* recommend transitioning to the FishLife prior within the current 2021 model framework. Though the prior on its own is likely a better represents the uncertainty in POP life history, this decision decision should not be made without concurrently revisiting the treatment of selectivity, and potential re-weighting of fishery length composition data. Given our findings above, we recommend this be undertaken within the context of a new modeling framework, where it has been revealed that the prior is less influential than other factors.

There remains conflict between likelihood components regarding the best estimate for $M$. This conflict is much more pronounced in the current POP model versus the SS model, both in terms of the range of $M$ values that are statistically indistinguishable, the discrepancy between MLEs for $M$ for each likelihood component, and the influence of the prior on the overall MLE. A visual inspection of the data reveals that the fishery length compositions appear nearly stable through time, while the age compositions from both sources appear to track cohorts to a greater degree. Secondary to the above recommendation, we will explore data weighting approaches in the new modeling framework. We re-iterate that the magnitude of disagreement between data sources appears less pronounced in the SS model.



```{r mposterior, fig.cap = "Left: Comparison of M priors (thick lines) and maximum likelihood estimates (thin vertical lines) between the base model (black), and the base model using a prior from the FishLife package (blue). Right: M priors from the FishLife taxa for POP and related taxa.", out.width="50%",out.height="50%"}
...
```{r ssmcompare, fig.cap = "Comparison of SSB trajectories for alternative SS configurations implemented for natural mortality explorations. The dotted black line is the 2021 POP Assessment.", out.width="30%",out.height="30%"}
knitr::include_graphics(here::here('2023','ss','v-300.00','ssb_compare_pt.png')) 
```

```{r mestimatess1, fig.cap = "Prior and posterior estimate of M from the SS model using  the new FishLife prior and logistic fishery selectivity; tuned compositional data weights using the Francis method have not been applied in this model.", out.width="30%",out.height="30%"}
knitr::include_graphics(here::here('2023','ss','v-300.01','plots','m_distribution.png'))
```

```{r mestimatess2, fig.cap = "Prior and posterior estimate of M from the SS model using  the new FishLife prior and logistic fishery selectivity; tuned compositional data weights using the Francis method have been applied in this model.", out.width="30%",out.height="30%"}
knitr::include_graphics(here::here('2023','ss','v-300.01-francis','plots','m_distribution.png')) 
```



```{r mestimatess3, fig.cap = "Prior and posterior estimate of M from the SS model using the original prior and logistic fishery selectivity; tuned compositional data weights using the Francis method have not been applied in this model", out.width="30%",out.height="30%"}
knitr::include_graphics(here::here('2023','ss','v-300.03','plots','m_distribution.png'))
```


```{r mestimatess4, fig.cap = "Prior and posterior estimate of M from the SS model using  the new FishLife prior and allowing fishery selectivity to be dome-shaped; tuned compositional data weights using the Francis method have not been applied in this model.", out.width="30%",out.height="30%"}
knitr::include_graphics(here::here('2023','ss','v-300.04','plots','m_distribution.png')) 
```
...
```{r mprofiless, fig.cap = "Likelihood profile on M using the SS Model. Values below the horizontal dashed line are statistically indistinguishable.", out.width="30%",out.height="30%"}
knitr::include_graphics(here::here('2023','ss','v-300.01f-natMprofile','profile_plot_likelihood.png'))  
```

```{r pinerplotmss, fig.cap = "Breakdown of likelihood profiles on M by age-composition fleet, using the SS model. Values below the horizontal dashed line are statistically indistinguishable..", out.width="30%",out.height="30%"}
knitr::include_graphics(here::here('2023','ss','v-300.01f-natMprofile','piner_plot_age.png')) 
```
