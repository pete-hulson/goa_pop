...
// Data and penalty weightings
  ...
  init_number          wt_recdev                               // Weight for estimation recruitment variatiions penalty
...
// Stock-recruitment
  ...
  init_bounded_vector  mean_rec_dev(styr+1,endyr,-10.,10.,ph_recdev+1);
  vector               log_mean_rec_vec(styr,endyr);
...
// Trawl Survey selectivity
  ...
  init_bounded_vector  log_q_dev(2,nyrs_srv1,-10.,10.,ph_q_srv1+1); // Catchability deviations
  vector               log_q_vec(1,nyrs_srv1);
  sdreport_vector      q_srv1(1,nyrs_srv1);                                 // " " for Survey1 catchability
...
// Likelihoods and penalty functions
  ...
  number               mean_recdev_like;                       // Likelihood value for mean recruitments
...
// Bottom Trawl Survey Catchability
  log_q_vec(1) = log_q_srv1;
  for (i=2;i<=nyrs_srv1;i++)
    log_q_vec(i) = log_q_vec(i-1) + log_q_dev(i);
  q_srv1 = exp(log_q_vec);
...
// Remaining years
  log_mean_rec_vec(styr) = log_mean_rec;
  for (i=styr+1;i<=endyr;i++)
  	log_mean_rec_vec(i) = log_mean_rec_vec(i-1) + mean_rec_dev(i);

  for (i=styr;i<endyr;i++) {
    natage(i,1) = mfexp(log_rec_dev(i) + log_mean_rec_vec(i));
...
// End year abundance
  natage(endyr,1) = mfexp(log_rec_dev(endyr) + log_mean_rec_vec(endyr)); 
...
// Calculate predicted data values
  ...
  for (i=1;i<=nyrs_srv1;i++)
    pred_srv1(i) = q_srv1(i) * (natage(yrs_srv1(i))*elem_prod(srv1_sel,wt));   // Predicted Survey biomass
...
// Sum objective function
  ...
  obj_fun           += wt_recdev * mean_recdev_like; 
...
// Calculate prior penalties
    ...
    if (active(log_q_dev))
      priors(4)    = norm2(log_q_dev)/(2.*square(cvq_devs));
...
//==============================================================================================================================
  ...
  rec_like.initialize();
  mean_recdev_like.initialize();

  rec_like = norm2(log_rec_dev)/(2*square(sigr)) + (size_count(log_rec_dev)*log(sigr));
  mean_recdev_like = norm2(mean_rec_dev)/(2*square(cvlog_mean_rec));
...
// Output file (.rep) which is loaded into R for data outputs
  ...
  report<<"  sigr   "<<endl;  
  report<<sigr<<endl;  
  report<<"   log_mean_rec_init"<<endl;
  report<<log_mean_rec<<endl;
  report<<"   log_mean_rec_vec"<<endl;
  report<<log_mean_rec_vec<<endl;
  report<<"   q_alt  "<<endl;
