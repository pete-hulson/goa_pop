...
// Numbers at age
  ...
  vector               rec_devs_early(styr,2011);              // Recruitment deviations from before the asssessment starts to present
  vector               rec_devs_late(2012,endyr);              // Recruitment deviations from before the asssessment starts to present
...
// End year abundance
  natage(endyr,1) = mfexp(log_rec_dev(endyr) + log_mean_rec2); 
...
// Calculate predicted data values
  ...
  for (i=11;i<=nyrs_srv1;i++)
    pred_srv1(i) = q_srv2 * (natage(yrs_srv1(i))*elem_prod(srv1_sel,wt));   // Predicted Survey biomass
...
// Calculate prior penalties
    priors.initialize();
    if (active(sigr1))
      priors(1)    = square(log(sigr1/sigrprior))/(2.*square(cvsigrprior)) + square(log(sigr2/sigrprior))/(2.*square(cvsigrprior));
...
//==============================================================================================================================
  ...
  rec_like = norm2(rec_devs_early)/(2*square(sigr1)) + (size_count(rec_devs_early)*log(sigr1)) + norm2(rec_devs_late)/(2*square(sigr2)) + (size_count(rec_devs_late)*log(sigr2));
...
// Output file (.rep) which is loaded into R for data outputs
  ...
  report<<"   q_trawl early   "<<endl;
  report<<q_srv1<<endl;
  report<<"   q_trawl late   "<<endl;
  report<<q_srv2<<endl;
  report<<"   nat_mort  "<<endl;
  report<<natmort<<endl;
  report<<"  sigr (styr-2011)  "<<endl;  
  report<<sigr1<<endl;  
  report<<"  sigr (2012-endyr)  "<<endl;  
  report<<sigr2<<endl;  
  report<<"   log_mean_rec1 (styr-2011)"<<endl;
  report<<log_mean_rec1<<endl;
  report<<"   log_mean_rec2 (2012-endyr)"<<endl;
