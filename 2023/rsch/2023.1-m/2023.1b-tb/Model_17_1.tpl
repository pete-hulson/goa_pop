...
// Predicted values
  ...
  init_number          logm1(ph_m);                             // Estimate log natural mortality
  init_number          logm2(ph_m);                             // Estimate log natural mortality
  init_number          logm3(ph_m);                             // Estimate log natural mortality
  number               q_cpue;
...
// Standard deviation estimates for some estimated parameters
  ...
  vector               natmort(styr,endyr);                                // " " for natural mortality
  sdreport_number      LMR;
...
//==============================================================================================================================
  ...
  for (i=styr;i<1978;i++)
    natmort(i) = mfexp(logm1);
  for (i=1978;i<2000;i++)
    natmort(i) = mfexp(logm2);
  for (i=2000;i<=endyr;i++)
    natmort(i) = mfexp(logm3);                                       // setting natural mortality to arithmetic scale
  if(ph_m>0) nattymort=natmort(endyr); else nattymort=log_mean_rec;
  ...
  for (int i=styr;i<=endyr;i++)
    Z(i) = F(i) + natmort(i);                                             // Fully selected total mortality
...
// Start year abundance
  ...
  for (j=2;j<nages_M;j++) {
    itmp = styr+1-j;
    natage(styr,j) = mfexp(log_mean_rec - natmort(styr) * double(j-1)+ log_rec_dev(itmp)); 
  }
  natage(styr,nages_M) = mfexp(log_mean_rec - natmort(styr) * (nages_M-1)) / (1. - exp(-natmort(styr)));
...
//==============================================================================================================================
  ...
  for (j=2;j<nages_M;j++) {
    Nspr(1,j)=Nspr(1,j-1)*mfexp(-1.*natmort(endyr));
    Nspr(2,j)=Nspr(2,j-1)*mfexp(-1.*(natmort(endyr)+mF50*fish_sel4(j-1)));
    Nspr(3,j)=Nspr(3,j-1)*mfexp(-1.*(natmort(endyr)+mF40*fish_sel4(j-1)));
    Nspr(4,j)=Nspr(4,j-1)*mfexp(-1.*(natmort(endyr)+mF35*fish_sel4(j-1)));
  }

  Nspr(1,nages_M)=Nspr(1,nages_M-1)*mfexp(-1.*natmort(endyr))/(1.-mfexp(-1.*natmort(endyr)));
  Nspr(2,nages_M)=Nspr(2,nages_M-1)*mfexp(-1.* (natmort(endyr)+mF50*fish_sel4(nages_M-1)))/(1.-mfexp(-1.*(natmort(endyr)+mF50*fish_sel4(nages_M))));
  Nspr(3,nages_M)=Nspr(3,nages_M-1)*mfexp(-1.* (natmort(endyr)+mF40*fish_sel4(nages_M-1)))/ (1.-mfexp(-1.*(natmort(endyr)+mF40*fish_sel4(nages_M))));
  Nspr(4,nages_M)=Nspr(4,nages_M-1)*mfexp(-1.* (natmort(endyr)+mF35*fish_sel4(nages_M-1)))/ (1.-mfexp(-1.*(natmort(endyr)+mF35*fish_sel4(nages_M))));
  
  for (j=1;j<=nages_M;j++) {
   // Kill them off till (spawn_fract)
    SB0    += Nspr(1,j)*wt_mature(j)*mfexp(-spawn_fract*natmort(endyr));
    SBF50  += Nspr(2,j)*wt_mature(j)*mfexp(-spawn_fract*(natmort(endyr)+mF50*fish_sel4(j)));
    SBF40  += Nspr(3,j)*wt_mature(j)*mfexp(-spawn_fract*(natmort(endyr)+mF40*fish_sel4(j)));
    SBF35  += Nspr(4,j)*wt_mature(j)*mfexp(-spawn_fract*(natmort(endyr)+mF35*fish_sel4(j)));
  }
...
// Abundance for remaining age classes in endyr+1
  ...
  spawn_biom_proj(endyr+1) = elem_prod(N_proj(endyr+1),pow(mfexp(-yieldratio * FABC_tot_proj-natmort(endyr)),spawn_fract)) * wt_mature;
  ...
  for (i=endyr+1;i<=endyr+15;i++) {
   ...
   // F ABC 
    ...
    for (j=1;j<=nages_M;j++) {  
      ...
      Z_proj(j) = FABC_tot_proj(j) + natmort(endyr);
      ZOFL_proj(j) = FOFL_tot_proj(j) + natmort(endyr);
   ...
   // Next year's abundance
    if (i < endyr + 15) {
     ...
     for (j=1; j<nages_M-1;j++) {
       N_proj(i+1,j+1) = N_proj(i,j) * mfexp(-yieldratio*FABC_tot_proj(j)-natmort(endyr)); }
     N_proj(i+1,nages_M) = N_proj(i,nages_M-1) * mfexp(-yieldratio*FABC_tot_proj(nages_M-1)-natmort(endyr))+ N_proj(i,nages_M) * mfexp(-yieldratio*FABC_tot_proj(nages_M)-natmort(endyr));
     spawn_biom_proj(i+1) = elem_prod(N_proj(i+1),pow(mfexp(-yieldratio*FABC_tot_proj-natmort(endyr)),spawn_fract)) * wt_mature;
...
// Calculate prior penalties
    ...
    if (active(logm1))
      priors(3)    = square(logm1-log(mprior))/(2.*square(cvmprior)) + square(logm2-log(mprior))/(2.*square(cvmprior)) + square(logm3-log(mprior))/(2.*square(cvmprior));
...
// Output file (.rep) which is loaded into R for data outputs
  ...
  report<<"   q_trawl   "<<endl;
  report<<q_srv1<<endl;
  report<<"   nat_mort (styr-1976)  "<<endl;
  report<<mfexp(logm1)<<endl;
  report<<"   nat_mort (1977-1999)  "<<endl;
  report<<mfexp(logm2)<<endl;
  report<<"   nat_mort (2000-endyr)  "<<endl;
  report<<mfexp(logm3)<<endl;  
  report<<"  sigr   "<<endl;  
  report<<sigr<<endl;  
...
// Function to write out data file for new Ianelli 2005 projection model....
 ...
 newproj <<"1"<<endl;
 for (j=1;j<=nages_M;j++) natmortv = natmort(endyr); 
