...
// Predicted values
  ...
  init_bounded_vector  m_devs(styr+1,endyr,-1.,1.,ph_m+1)
  number               q_cpue;
...
//==============================================================================================================================
  ...
  natmort(styr) = mfexp(logm);                                       // setting natural mortality to arithmetic scale
  for (int i=styr+1;i<=endyr;i++)
    natmort(i) = mfexp(log(natmort(i-1)) + m_devs(i));

  if(ph_m>0) nattymort=sum(natmort)/nyrs; else nattymort=log_mean_rec;
  ...
  for (i=endyr+1;i<=endyr+15;i++) {
   ...
   // Next year's abundance
    if (i < endyr + 15) {
     ...
     N_proj(i+1,nages_M) = N_proj(i,nages_M-1) * mfexp(-yieldratio*FABC_tot_proj(nages_M-1)-natmort(endyr))+ N_proj(i,nages_M) * mfexp(-yieldratio*FABC_tot_proj(nages_M)-(endyr));
...
// Calculate prior penalties
    ...
    if (active(logm))
    for (int i=styr;i<=endyr;i++)
      priors(3)    += square(log(natmort(i))-log(mprior))/(2.*square(cvmprior));
